...
/* Offsets (верные для PY32F0/G0) */
#define PUYA_FLASH_ACR     0x00
#define PUYA_FLASH_KEYR    0x08
#define PUYA_FLASH_OPTKEYR 0x0C
#define PUYA_FLASH_SR      0x10
#define PUYA_FLASH_CR      0x14
#define PUYA_FLASH_OPTR    0x20   /* Option bytes register */
#define PUYA_FLASH_SDKR    0x24   /* Security/Debug Key register */
#define PUYA_FLASH_WRPR    0x2C   /* Write protection register */

/* Абсолютные адреса (если где-то нужны целиком): */
#define PUYA_FLASH_KEYR_B0    (FLASH_REG_BASE + PUYA_FLASH_KEYR)     /* 0x40022008 */
#define PUYA_FLASH_OPTKEYR_B0 (FLASH_REG_BASE + PUYA_FLASH_OPTKEYR)  /* 0x4002200C */
#define PUYA_FLASH_SR_B0      (FLASH_REG_BASE + PUYA_FLASH_SR)       /* 0x40022010 */
#define PUYA_FLASH_CR_B0      (FLASH_REG_BASE + PUYA_FLASH_CR)       /* 0x40022014 */
#define PUYA_FLASH_OPTR_B0    (FLASH_REG_BASE + PUYA_FLASH_OPTR)     /* 0x40022020 */
#define PUYA_FLASH_WRPR_B0    (FLASH_REG_BASE + PUYA_FLASH_WRPR)     /* 0x4002202C */
...
#define OPTKEY1         0x08192A3Bu      /* для FLASH_OPTKEYR — заменить */
...
static int puya_wait_and_clear_eop(struct flash_bank *bank, int timeout_ms, bool expect_eop)
{
	...
	while (1) {
		rv = target_read_u32(t, puyaf0x_get_flash_reg(bank, PUYA_FLASH_SR), &sr);
		if (rv != ERROR_OK) return rv;
		if ((sr & FLASH_BSY) == 0) break;
		if (timeout_ms-- <= 0) {
			LOG_ERROR("timeout waiting FLASH busy clear");
			return ERROR_FLASH_BUSY;
		}
		alive_sleep(1);
	}

	if (expect_eop && !(sr & FLASH_EOP)) {
		LOG_ERROR("option programming failed (no EOP)");
		return ERROR_FLASH_OPERATION_FAILED;
	}

	/* Сбросить EOP и возможные ошибки опций */
	if (sr & (FLASH_EOP | FLASH_OPTVERR)) {
		rv = target_write_u32(t, puyaf0x_get_flash_reg(bank, PUYA_FLASH_SR),
							  (sr & (FLASH_EOP | FLASH_OPTVERR)));
		if (rv != ERROR_OK) return rv;
	}
	return ERROR_OK;
}
...
{
	struct puyaf0x_flash_bank *info = bank->driver_priv;
	struct target *t = bank->target;
	uint32_t optr;
	uint32_t wrpr;
	int retval;

	/* Прочитать регистр опций */
	retval = target_read_u32(t, puyaf0x_get_flash_reg(bank, PUYA_FLASH_OPTR), &optr);
	if (retval != ERROR_OK)
		return retval;

	/* Прочитать регистр защиты */
	retval = target_read_u32(t, puyaf0x_get_flash_reg(bank, PUYA_FLASH_WRPR), &wrpr);
	if (retval != ERROR_OK)
		return retval;

	/* Распечатать сырые регистры (для удобства отладки) */
	LOG_INFO("FLASH_OPTR = 0x%08" PRIx32 ", FLASH_WRPR = 0x%08" PRIx32, optr, wrpr);

	/* Разбор полей OPTR */
	info->option_bytes.rdp = optr & 0xFF;           /* RDP[7:0] */
	bool bor_en = (optr >> 8) & 0x1;
	uint8_t bor_lev = (optr >> 9) & 0x7;
	bool iwdg_sw = (optr >> 12) & 0x1;
	uint8_t nrst_mode = (optr >> 13) & 0x3;
	bool nboot1 = (optr >> 15) & 0x1;

	/* Сохранение защиты страниц */
	info->option_bytes.protection = wrpr;

	/* Вывод значений в лог */
	LOG_INFO("Read Protection (RDP): 0x%02" PRIx8, info->option_bytes.rdp);
	LOG_INFO("BOR enabled: %s, BOR level: %u", bor_en ? "yes" : "no", bor_lev);
	LOG_INFO("IWDG mode: %s", iwdg_sw ? "software" : "hardware");
	LOG_INFO("nRST mode: %u", nrst_mode);
	LOG_INFO("nBOOT1: %s", nboot1 ? "1" : "0");
	LOG_INFO("Write Protection bits: 0x%X", wrpr);

	return ERROR_OK;
}

static inline uint32_t puya_pack_optr(const struct puyaf0x_flash_bank *info)
{
	uint32_t optr = 0;
	optr |= (uint32_t)(info->option_bytes.rdp & 0xFF);          /* RDP[7:0] */
	/* user: bit0 -> IWDG_SW(13), bit1 -> nRST_STOP(12), bit2 -> nRST_STDBY(11) — пример  */
	if (info->option_bytes.user & (1U << 0)) optr |= (1U << 13);
	if (info->option_bytes.user & (1U << 1)) optr |= (1U << 12);
	if (info->option_bytes.user & (1U << 2)) optr |= (1U << 11);
	return optr;
}
...
static int puyaf0x_erase_options(struct flash_bank *bank)
{
	...
	/* 1) Обычный unlock FLASH_CR (KEYR) */
	rv = target_write_u32(t, puyaf0x_get_flash_reg(bank, PUYA_FLASH_KEYR), KEY1);
	if (rv != ERROR_OK) return rv;
	rv = target_write_u32(t, puyaf0x_get_flash_reg(bank, PUYA_FLASH_KEYR), KEY2);
	if (rv != ERROR_OK) return rv;

	/* 2) Unlock опций (OPTKEYR) особенными ключами */
	rv = target_write_u32(t, puyaf0x_get_flash_reg(bank, PUYA_FLASH_OPTKEYR), OPTKEY1);
	if (rv != ERROR_OK) goto opt_lock;
	rv = target_write_u32(t, puyaf0x_get_flash_reg(bank, PUYA_FLASH_OPTKEYR), OPTKEY2);
	if (rv != ERROR_OK) goto opt_lock;

	/* 3) «Стереть» = записать дефолты и стартовать программирование */
	/* Сброс читаемой защиты на L0 (обычно A5h для Puya), user/data/protection — по вашему политике. */
	info->option_bytes.rdp = info->default_rdp;      /* например 0xA5 */
	uint32_t optr = puya_pack_optr(info);

	rv = target_write_u32(t, puyaf0x_get_flash_reg(bank, PUYA_FLASH_OPTR), optr);
	if (rv != ERROR_OK) goto opt_lock;
	rv = target_write_u32(t, puyaf0x_get_flash_reg(bank, PUYA_FLASH_SDKR), info->option_bytes.data);
	if (rv != ERROR_OK) goto opt_lock;
	rv = target_write_u32(t, puyaf0x_get_flash_reg(bank, PUYA_FLASH_WRPR), info->option_bytes.protection & 0xFFFF);
	if (rv != ERROR_OK) goto opt_lock;

	/* Старт программирования опций */
	rv = target_write_u32(t, puyaf0x_get_flash_reg(bank, PUYA_FLASH_CR), FLASH_OPTSTRT);
	if (rv != ERROR_OK) goto opt_lock;

	rv = puya_wait_and_clear_eop(bank, FLASH_ERASE_TIMEOUT, true);

	opt_lock:
		{
			int rv2 = target_write_u32(t, puyaf0x_get_flash_reg(bank, PUYA_FLASH_CR), FLASH_OPTLOCK);
			if (rv == ERROR_OK) rv = rv2;
		}
	return rv;
}
...
static int puyaf0x_write_options(struct flash_bank *bank)
{
	...
	/* 0) На всякий случай дождаться готовности */
	rv = puya_wait_and_clear_eop(bank, FLASH_PRE_TIMEOUT, false);
	if (rv != ERROR_OK) return rv;
	...
	/* 2) Unlock опций (OPTKEYR) особыми ключами */
	rv = target_write_u32(t, puyaf0x_get_flash_reg(bank, PUYA_FLASH_OPTKEYR), OPTKEY1);
	if (rv != ERROR_OK) goto opt_lock;
	rv = target_write_u32(t, puyaf0x_get_flash_reg(bank, PUYA_FLASH_OPTKEYR), OPTKEY2);
	if (rv != ERROR_OK) goto opt_lock;

	/* 3) Записать новые значения OPTR/SDKR/WRPR */
	uint32_t optr = puya_pack_optr(info);
	rv = target_write_u32(t, puyaf0x_get_flash_reg(bank, PUYA_FLASH_OPTR), optr);
	if (rv != ERROR_OK) goto opt_lock;
	...
	/* 4) Стартовать программирование опций */
	rv = target_write_u32(t, puyaf0x_get_flash_reg(bank, PUYA_FLASH_CR), FLASH_OPTSTRT);
	if (rv != ERROR_OK) goto opt_lock;

	/* 5) Дождаться завершения и очистить EOP */
	rv = puya_wait_and_clear_eop(bank, FLASH_WRITE_TIMEOUT, true);
...
static int puyaf0x_get_flash_size(struct flash_bank *bank, uint16_t *kib)
{
	struct target *t = bank->target;
	uint32_t addr = 0x1FFF0FFC; /* PY32 Flash size register */
	return target_read_u16(t, addr, kib);
}
...
COMMAND_HANDLER(puyaf0x_handle_options_read_command)
{
	...
	uint32_t optr, wrpr;
	retval = target_read_u32(target, puyaf0x_get_flash_reg(bank, PUYA_FLASH_OPTR), &optr);
	if (retval != ERROR_OK)
		return retval;

	retval = target_read_u32(target, puyaf0x_get_flash_reg(bank, PUYA_FLASH_WRPR), &wrpr);
	if (retval != ERROR_OK)
		return retval;

	/* Распаковываем поля из OPTR */
	...
	command_print(CMD, "Option byte register (OPTR) = 0x%08" PRIx32, optr);
	command_print(CMD, "Write protection register (WRPR) = 0x%08" PRIx32, wrpr);

	command_print(CMD, "Read Protection (RDP): 0x%02" PRIx8, rdp);
	command_print(CMD, "BOR enabled: %s, BOR level: %u", bor_en ? "yes" : "no", bor_lev);
	command_print(CMD, "IWDG mode: %sware", iwdg_sw ? "soft" : "hard");
	command_print(CMD, "nRST mode (bits): 0x%X", nrst_mode);
	command_print(CMD, "nBOOT1: %s", nboot1 ? "1" : "0");
	command_print(CMD, "Write Protection bits: 0x%X", wrpr);
